document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderChampions(champs) {
        ...
        champs.forEach(champ => {
            ...
            // Добавляем класс selected, если чемпион уже выбран
    ...
    function selectChampion(champ) {
        // Проверяем, есть ли уже этот чемпион в выбранных
        const index = state.selectedChampions.findIndex(c => c.id === champ.id);
        
        if (index >= 0) {
            // Если чемпион уже выбран, удаляем его
        ...
        updateSelectionUI();
        renderChampions(champions); // Перерисовываем сетку для обновления состояния выбранных чемпионов
        
        if (state.selectedChampions.length === 2) {
            analyzeMatchup();
        } else {
            // Если меньше 2 чемпионов выбрано, скрываем анализ
    ...
    function updateSelectionUI() {
        ...
        // Обновляем первый слот, если есть первый чемпион
        if (first) {
        ...
        // Обновляем второй слот, если есть второй чемпион
        if (second) {
